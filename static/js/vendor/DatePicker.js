define(
    ...
    function ($, Class, ResLoader, FunctionalMarker, Runner) {
        ResLoader.loadCSS(__uri("/static/css/vendor/jquery/ui-lightness/jquery-ui-1.10.3.custom.min.css"));
        var jQuery = $;
    ...
    $.timepicker.version = "1.4";
        ...
        FunctionalMarker.regist({
            ...
            init: function (element, params) {
                ...
                var $element = $(element).datetimepicker(params);
                
                $element.attr("autocomplete", "off");
                $element.attr("readonly", "readonly");
                
                var $element = $(element);
                clearButton = $("<button type='button' style='position: absolute;'>清空</button>");
                $element.data("clearButton", clearButton);
        ...
        (new Runner(function () {
            for(var i = 0, l = inputList.length; i < l; ++ i)
            {
                ...
                var _button = _input.data("clearButton");
                var offset = _input.offset();
                var width = _input.outerWidth();
                var right = offset.left + width;
                _button.css({
                    position: "absolute",
                    height: _input.innerHeight() + "px",
                    left: right - _button.outerWidth() + "px",
                    top: offset.top + "px"
                });
            }
        }, 1)).start();
